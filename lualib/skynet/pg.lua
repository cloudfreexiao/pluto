...
local scram_sha256 = require("scram")
...
local function parse_error(err_msg)
    ...
    while offset < err_len do                                 -- Stop before the final null terminator
        ...
        local null_pos = strfind(err_msg, NULL, offset, true) -- Find the next null terminator
...
local function md5_auth(self, msg)
    ...
    send_message(
        ...
        {
            "md5",
            md5.sumhexa(md5.sumhexa(self.password .. self.user) .. salt),
            NULL
        }
    )
    return check_auth(self)
...
function pg.connect(opts)
    local sock, err = socket.open(opts.host, opts.port)
    if not sock then
